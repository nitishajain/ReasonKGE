...
#import kge.util
#import kge.model.kge_model
#from model.kge_model import KgeModel

#from .io import load_checkpoint - works but gives error in next step
#from kge.model import KgeModel - gives error related to load_checkpoint
...
class KgeSamplerReasoning(Configurable):
    ...
    def create(
        ...
        print('Creating the KgeSamplerReasoning object ')

        sampling_type = config.get(configuration_key + ".sampling_type")
        if sampling_type == "uniform":
            return KgeUniformSamplerReasoning(config, configuration_key, dataset)
        elif sampling_type == "frequency":
            return KgeFrequencySamplerReasoning(config, configuration_key, dataset)
        else:
    ...
    def sample(
        ...
        #print("Sampling code starts")
        #print (self.shared, self.filter_positives)
        ...
        if self.filter_positives[slot]:
            #print(self.filter_implementation)
        ...
        #check the type of sampling here, if reasoning, then retunr the new reasoning object I have created
        # need to implement samples for that and check if it is becing called or not

        def reasoner_negatives_samples(triples, negative_samples, reasoner_samples_map):

            # now creating the numpy from the original negative triples calculated earlier in the samples()
            # convert to numpy, go through each row and replace by my negative triple if index matches
            #print(negative_samples.shape)
            object_numpy = negative_samples.numpy()

            #print("Created numpy", object_numpy.size())
            #print('Original', object_numpy[:5])
            #print("To be replaced:", reasoner_samples_map.keys())
            #print("Replacing numpy values with my negative samples")
            replaced = 0
            for i in range(len(object_numpy)):
                if i in reasoner_samples_map:
                    replaced += 1
                    for idx in range(len(reasoner_samples_map[i])):
                        object_numpy[i][idx] = reasoner_samples_map[i][idx]

            #print('Updated', object_numpy[:5])
            print("Triples replaced - ", replaced)
            object_tensor = torch.from_numpy(object_numpy)

            # print("Updated negative samples for this slot:")
            # print(object_tensor.shape)
            # print(object_tensor)
            return object_tensor
            # exit(0) #for stopping at one batch

        def abstracted_negatives_samples(triples, negative_samples, reasoner_samples_map):
            # now creating the numpy from the original negative triples calculated earlier in the samples()
            # convert to numpy, go through each row and replace by my negative triple if index matches
            #print(negative_samples.shape)
            object_numpy = negative_samples.numpy()
            ...
            replaced = 0
            for i in range(len(object_numpy)):
                #if there are any samples generated from reasoner
                if i in reasoner_samples_map:
                    replaced += 1
                    #the len of samples could be greater or smaller than num_samples
                    #if len is less, then simply replace with the reasoner samples, starting from 0
                    flat_reasoner_samples = [sample for sublist in reasoner_samples_map[i] for sample in sublist]
                    #print(len(flat_reasoner_samples))

                    if len(flat_reasoner_samples) <= num_samples:
                        #flatten out the list of lists first, then do this
                        for idx in range(len(flat_reasoner_samples)):
                            object_numpy[i][idx] = flat_reasoner_samples[idx]

                    else:

                        #if len is more, then all reasoner samples cannot be included,
                        # include up till num_samples i.e. length of the default samples
                        #flatten out here as well

                        # for idx in range(num_samples):
                        #     object_numpy[i][idx] = flat_reasoner_samples[idx]


                        #alternatively, pick up samples from each list in round robin fashion
                        #this will ensure equal represenatation of the abstract samples


                        #reasoner_samples_map = [a:[1, 2, 3], [1, 5, 6], [7, 8, 9]]
                        idx = 0  # pointer index, starting from beginning
                        slots_tobe_filled = set()
                        while len(slots_tobe_filled) < num_samples:
                            #print("Reasoner_samples_map[i]", i, len(reasoner_samples_map[i]), len(slots_tobe_filled))
                            for list_samples in reasoner_samples_map[i]:
                                #print (len(list_samples))
                                if idx >= len(list_samples):  # Current list is shorter than iterator index
                                    continue
                                slots_tobe_filled.add(list_samples[idx])  # add element to slot
                                if len(slots_tobe_filled) == num_samples:  # terminate early if desired slot size is achieved
                                    break
                            idx += 1  # increment pointer index by 1 once you've gone through all lists

                        #print(slots_tobe_filled)
                        object_numpy[i] = list(slots_tobe_filled)
        ...
        #print('Negative samples generated ', negative_samples)

        #print("first", first_epoch, type(first_epoch))

        if not first_epoch:
            #print("Sampling code..")
            if (abstraction == 'False'):
                #pass
                #no need for checking slot now, always call the function directly for replacing samples
                negative_samples = reasoner_negatives_samples(positive_triples, negative_samples, reasoner_samples_map)
            else:
                #pass
                #print("Replacing negative samples")
                #print("Replacing triples:", len(reasoner_samples_map.keys()))
                negative_samples = abstracted_negatives_samples(positive_triples, negative_samples, reasoner_samples_map)

        else:
            print("Samples found for triples:", len(reasoner_samples_map.keys()) )


        #print('Now returning the DefaultBatchReasoningSample object ')
        return DefaultBatchReasoningSample(
            self.config,
...
class KgeSampler(Configurable):
    ...
    def sample(
        ...
        #print('Returning the DefaultBatchNegativeSample object ')
...
class DefaultBatchNegativeSample(BatchNegativeSample):
    ...
    def samples(self, indexes=None) -> torch.Tensor:
        #print('Inside the samples fn of DefaultBatchNegativeSample')
...
class DefaultBatchReasoningSample(BatchNegativeSample):
    """Default implementation that stores all negative samples as a tensor.
        This class will serve as the one having custom reasonng based negative samples tensor"""
    ...
    def samples(self, indexes=None) -> torch.Tensor:
        #only returns the negative tensor directly for non shared implementations
        #no need to add any code here then, only directly modify the negative tensor in samples
        #print('sample() - Getting the negative samples from DefaultBatchReasoningSample')
        #print(self._samples)

        return self._samples if indexes is None else self._samples[indexes]

    def to(self, device) -> "DefaultBatchReasoningSample":
        super().to(device)
        self._samples = self._samples.to(device)
        return self
...
class KgeUniformSamplerReasoning(KgeSamplerReasoning):
    def __init__(self, config: Config, configuration_key: str, dataset: Dataset):
        super().__init__(config, configuration_key, dataset)
        print('KgeUniformSamplerReasoning')
...
class KgeFrequencySamplerReasoning(KgeSamplerReasoning):
    """
    Sample negatives based on their relative occurrence in the slot in the train set.
    Can be smoothed with a symmetric prior.
    """
    ...
    def __init__(self, config, configuration_key, dataset):
        super().__init__(config, configuration_key, dataset)
        print('KgeFrequencySamplerReasoning')
        self._multinomials = []
        alpha = self.get_option("frequency.smoothing")
        for slot in SLOTS:
            smoothed_counts = (
